...
def main() -> None:
    ...
    st.caption("Начальная версия · просмотр реальной выгрузки вашего региона")
    st.info(
        "Каркас помогает исследовать исходные записи. Добавьте собственные вопросы, "
        "проверки гипотез, карты, проекции и обоснованные выводы по условию ДЗ."
    )
    ...
    st.caption(
        ...
        "Все показатели ниже относятся к текущему срезу. Считаются исходные записи, "
    ...
    if len(current) < 30:
        st.warning(
            "В срезе меньше 30 записей. Это подсказка каркаса, а не универсальный "
            "статистический порог; обоснуйте своё правило для малых групп."
        )
    ...
    with overview:
        ...
        st.caption(
            "Ноль означает отсутствие записей в выбранной выгрузке и срезе. "
            "Первый и последний месяцы могут быть неполными. "
            "Полноту регистрации и сопоставимость периодов нужно проверить отдельно."
        )
    with geography:
